...
async function seedDatabase() {
  try {
    ...
    // Obtener IDs de usuarios reales para evitar errores de FK
    const adminUser = await pool.query(
      "SELECT id FROM users WHERE email = 'admin@taskflow.com'",
    );
    const managerUser = await pool.query(
      "SELECT id FROM users WHERE email = 'manager@taskflow.com'",
    );
    const user1 = await pool.query(
      "SELECT id FROM users WHERE email = 'user@taskflow.com'",
    );
    const user2 = await pool.query(
      "SELECT id FROM users WHERE email = 'user2@taskflow.com'",
    );
    ...
    if (!adminId || !managerId) {
      throw new Error(
        "❌ No se encontraron los usuarios base (admin/manager). Ejecuta db:reset primero.",
      );
    }
    ...
    const projectUserAssignments = [
      ...
      { project_id: projectIds[0], user_id: adminId },
      { project_id: projectIds[0], user_id: managerId },
      { project_id: projectIds[0], user_id: user1Id },
      { project_id: projectIds[0], user_id: user2Id },

      // Mobile App (Manager + User2 + Admin)
      { project_id: projectIds[1], user_id: adminId },
      { project_id: projectIds[1], user_id: managerId },
      { project_id: projectIds[1], user_id: user2Id }, // User2 leads mobile?

      // API Integration (Manager + User1 + User2)
      { project_id: projectIds[2], user_id: managerId },
      { project_id: projectIds[2], user_id: user1Id },
      { project_id: projectIds[2], user_id: user2Id },

      // Database Migration (Admin + Manager + User1)
      { project_id: projectIds[3], user_id: adminId },
      { project_id: projectIds[3], user_id: managerId },
      { project_id: projectIds[3], user_id: user1Id },
    ];
    ...
    const taskUserAssignments = [
      ...
      { task_id: taskIds[0], user_id: user1Id },
      { task_id: taskIds[0], user_id: user2Id },
      { task_id: taskIds[1], user_id: managerId },
      { task_id: taskIds[2], user_id: user1Id },

      // Mobile App tasks
      { task_id: taskIds[3], user_id: adminId },
      { task_id: taskIds[4], user_id: user2Id },
      { task_id: taskIds[5], user_id: user2Id },

      // API Integration tasks
      { task_id: taskIds[6], user_id: managerId },
      { task_id: taskIds[7], user_id: user1Id },
      { task_id: taskIds[8], user_id: user1Id },
      { task_id: taskIds[8], user_id: user2Id },

      // Database Migration tasks
      { task_id: taskIds[9], user_id: adminId },
      { task_id: taskIds[10], user_id: managerId },
      { task_id: taskIds[11], user_id: adminId },
    ];
